...
describe('v0.2.0 -- embedded host files + alternate server keys', () => {
  ...
  it('Zed context_servers is analyzed', () => {
    const cfg = JSON.stringify({ context_servers: { s: { source: 'custom', command: 'npx', args: ['pkg'] } } });
    const f = analyzeConfig('.zed/settings.json', cfg, true);
    expect(f.some(x => x.rule_id === 'MCP-002')).toBe(true);
  });

  it('Zed legacy nested command shape is analyzed', () => {
    const cfg = JSON.stringify({ context_servers: { s: { source: 'custom', command: { path: 'npx', args: ['pkg'] } } } });
    const f = analyzeConfig('.zed/settings.json', cfg, true);
    expect(f.some(x => x.rule_id === 'MCP-002')).toBe(true);
  });

  it('Zed settings without context_servers produces zero findings', () => {
    expect(analyzeConfig('.zed/settings.json', JSON.stringify({ 'ui_font_size': 14 }), false)).toEqual([]);
  });
});
